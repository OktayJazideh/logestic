...
DONE_ANCHORS = frozenset({
    ...
    "wf-queue-1", "wf-intransit-1", "hh-kyc-committee-1",
    "hourly-rej-1",
    "consult-ui-1",
    "need-hourly-1",
    "hourly-app-1",
...
DONE_BANNER = (
    '  <p class="done-banner">✅ <strong>انجام شده</strong> — بازبینی کد ۱۴۰۵/۰۳/۰۱. '
    'جزئیات: <a href="mvp-flow-chat-master-report-fa-v4.html#done-archive">گزارش v4 § آرشیو</a> · '
    '<a href="mvp-flow-chat-master-report-fa-v3.html#done">v3 § done</a>.</p>\n'
)
...
<header>
  ...
  <p>مرجع: <a href="mvp-flow-chat-master-report-fa-v4.html">گزارش v4 (باقی‌مانده)</a> · <a href="mvp-flow-chat-master-report-fa-v3.html">v3 (آرشیو)</a> · <a href="mvp-flow-chat-master-report-fa-v4.html#client-message">متن کارفرما</a></p>
</header>

<div class="guide">
  <strong>راهنما:</strong> P0 فعلی: DEPLOY-SAHMAN + CLIENT-DATA. باسکول فاز ۱ = <strong>ورود دستی COOP_OPERATOR</strong> (Agent سخت‌افزار P2/scaffold). FIN-SEED-99-1 و HOLD-SETTLEMENT-1 ✓. کاربران دمو: <a href="demo-users-and-deploy-fa.md">demo-users-and-deploy-fa.md</a>.
</div>
...
add(
    "FIN-SEED-99-1",
    "P0",
    "Seed پایلوت 99/1 (جایگزین 98/2)",
    "fin-seed-99-1",
    "Freeze کارفرما: split عملیاتی 99% مالک + 1% پلتفرم. seed TAFTAN و test-pilot-taftan1 باید با mines.platform_fee_value=0.01 هم‌تراز باشند.",
    [
        "Community split جدا — این کارت فقط operational platform fee.",
        "RULE_DEFAULTS split.platform می‌تواند 0.01 شود ولی mine-level platform_fee_value مرجع پایلوت است.",
        "Regression: comm-ton1, fin-policy1, pilot-taftan1.",
    ],
    [
        "apps/backend/scripts/seedConstants.ts — TAFTAN_PLATFORM_FEE_VALUE",
        "apps/backend/scripts/seed.ts — upsertMine TAFTAN",
        "apps/backend/scripts/test-pilot-taftan1.ts",
    ],
    """پروژه: logestic — FIN-SEED-99-1 (P0)

## هدف
هم‌ترازی seed پایلوت تفتان با Freeze 99/1.

## گام‌ها
1. seedConstants: TAFTAN_PLATFORM_FEE_VALUE = 0.01
2. seed.ts: پس از upsertMine TAFTAN → prisma.mines.update platform_fee_value
3. test-pilot-taftan1.ts: assert owner 99% / platform 1%

## DoD
- npm run test:pilot-taftan1 ×3 pass
- npm run test:comm-ton1 + test:fin-policy1 pass""",
    ["TAFTAN seed platform_fee_value=0.01", "pilot-taftan asserts 99/1"],
    ["npm -w @app/backend run test:pilot-taftan1", "npm run test:comm-ton1", "npm run test:fin-policy1"],
)

add(
    "HOLD-SETTLEMENT-1",
    "P0",
    "فیلتر HELD از settlement close",
    "hold-settlement-1",
    "مأموریت payment_state=HELD یا ticket PENDING_HOLD نباید در monthlyClose/ownerWeeklyClose وارد خطوط تسویه شود.",
    [
        "Helper مرکزی missionEligibleForSettlementWhere در lib/settlementEligibility.ts.",
        "Defense in depth: financeLedgerRepository قبلاً hold flag دارد — settlement query اصلاح اصلی است.",
        "test-hold-settlement1.ts ×3.",
    ],
    [
        "apps/backend/src/lib/settlementEligibility.ts",
        "apps/backend/src/repositories/settlementRepository.ts",
        "apps/backend/scripts/test-hold-settlement1.ts",
    ],
    """پروژه: logestic — HOLD-SETTLEMENT-1 (P0)

## هدف
exclude missions where payment_state IN (HELD, FAILED) or weighbridge ticket PENDING_HOLD from settlement close queries.

## گام‌ها
1. settlementEligibility.ts — missionEligibleForSettlementWhere()
2. settlementRepository monthlyClose + ownerWeeklyClose — spread helper in where
3. test-hold-settlement1.ts — HELD + PENDING_HOLD scenarios
4. package.json test:hold-settlement1 + infra-regression:finance chain

## DoD
- npm run test:hold-settlement1 ×3 pass
- npm run test:infra-regression:finance pass""",
    ["settlementEligibility helper", "monthlyClose excludes HELD", "test-hold-settlement1"],
    ["npm -w @app/backend run test:hold-settlement1", "npm run test:infra-regression:finance"],
)

add(
    "NATID-CHECKSUM-REG-1",
    "P1",
    "checksum کد ملی در ثبت خانوار",
    "natid-checksum-reg-1",
    "bulk import checksum دارد؛ POST /households/register باید validateIranNationalIdChecksum قبل از assertNationalIdAvailable داشته باشد.",
    [
        "همان lib/nationalId.ts که bulk import استفاده می‌کند.",
        "خطای 400 invalid_national_id.",
    ],
    [
        "apps/backend/src/routes/households.ts",
        "apps/backend/src/lib/nationalId.ts",
    ],
    """پروژه: logestic — NATID-CHECKSUM-REG-1 (P1)

## هدف
POST /api/households/register — normalizeNationalId + validateIranNationalIdChecksum

## DoD
- invalid checksum → 400 invalid_national_id
- valid checksum → flow unchanged""",
    ["register route checksum validation"],
    ["manual POST register with bad checksum", "npm run test:kyc-reg1 if exists"],
)

add(
    "WB-LOCAL-AGENT-1",
    "P2",
    "Local Agent باسکول (فاز بعد — scaffold آماده)",
    "wb-local-agent-1",
    "تصمیم قفل UAT: فاز ۱ مسیر رسمی = COOP_OPERATOR وزن را در WeighbridgePage دستی وارد می‌کند. Agent سخت‌افزار optional — apps/weighbridge-agent/ scaffold + ingest API آماده برای وقتی معدن spec بدهد.",
    [
        "خارج blocker UAT فاز ۱.",
        "بدون خرید PC/Serial تا تأیید کارفرما.",
        "Scaffold: apps/weighbridge-agent/ + POST /api/weighbridge/ingest.",
    ],
    [
        "apps/weighbridge-agent/",
        "apps/backend/src/services/weighbridgeIngestService.ts",
        "apps/backend/src/routes/weighbridge.ts",
    ],
    """پروژه: logestic — WB-LOCAL-AGENT-1 (P0)

## مرجع: گزارش v4 #remaining — WB-LOCAL-AGENT-1

## هدف
Node service کنار باسکول:
- read serial/TCP (config per client spec)
- POST ingest با Idempotency-Key
- offline queue JSON + retry 30s

## ساختار
apps/weighbridge-agent/
  README.md, package.json, .env.example
  src/index.js — queue + postIngest
  src/mock-test.js — integration بدون hardware

## پس از دریافت spec باسکول از کارفرما
- parser وزن واقعی (regex/CSV/binary)
- Windows Service / systemd unit

## DoD
- README فارسی/انگلیسی deploy
- mock-test اجرا شود
- ingest واقعی با token staging یک ticket بسازد""",
    ["apps/weighbridge-agent scaffold", "README deploy", "mock-test"],
    ["cd apps/weighbridge-agent && npm run test:mock"],
)

add(
    "DEPLOY-SAHMAN-1",
    "P0",
    "استقرار production sahman.ir",
    "deploy-sahman-1",
    "اجرای deploy-runbook-fa.md روی Liara/VPS — DNS panel/api، SMS real، smoke UAT checklist §۳.",
    [
        "کارفرما تعهد داده — کد runbook آماده است.",
        "Staging اول با SMS mock توصیه می‌شود.",
    ],
    [
        "docs/deploy-runbook-fa.md",
        "docs/uat-handover-checklist-fa.md",
        ".github/workflows/ci.yml",
    ],
    """پروژه: logestic — DEPLOY-SAHMAN-1 (P0)

## مرجع: docs/deploy-runbook-fa.md

## گام‌ها
1. VPS/Liara — Postgres + backend + web static
2. env production: NODE_ENV=production, SMS_PROVIDER=real, secrets
3. DNS: sahman.ir, panel.sahman.ir, api.sahman.ir
4. prisma migrate deploy + db:seed TAFTAN
5. Smoke: uat-handover-checklist §۳

## DoD
- HTTPS live
- OTP SMS real (یا staging mock documented)
- pilot-taftan1 روی staging DB pass""",
    ["production deploy", "DNS", "smoke checklist"],
    ["uat-handover-checklist §۳", "curl health api"],
)

add(
    "CLIENT-DATA-PILOT-1",
    "P0",
    "داده‌های پایلوت از کارفرما (gate UAT)",
    "client-data-pilot-1",
    "UAT مالی معنی کامل بدون: fixed_community_amount، IBAN رسمی تعاونی، لیست روستا، spec باسکول، contacts.",
    [
        "عملیات کارفرما — در گزارش v4 § client-message لیست ۱۱ مورد.",
        "seed placeholder 400k ریال/تن تا تأیید.",
    ],
    [
        "apps/backend/scripts/seedConstants.ts",
        "apps/backend/prisma/schema.prisma — service_contracts",
    ],
    """پروژه: logestic — CLIENT-DATA-PILOT-1 (gate — mostly client)

## تحویل از کارفرما (checklist)
1. fixed_community_amount_rial_per_unit نهایی تفتان
2. نام رسمی تعاونی + IBAN حساب رسمی
3. مسئول عملیات + تاریخ هدف UAT
4. لیست روستاهای مصوب (Excel)
5. spec باسکول (model, serial/TCP)
6. لیست کاربران پایلوت (موبایل + نقش)
7. تأیید dispatch manual vs auto
8. تأیید برند سهمان

## پس از دریافت
- seedConstants + service_contracts update
- re-seed staging
- sign-off UAT""",
    ["client deliverables received", "seed updated with real contract amount"],
    ["npm -w @app/backend run db:seed", "test:pilot-taftan1"],
)

add(
    "BRAND-SAHMAN-1",
    "P1",
    "برند «سهمان» در UI و docs",
    "brand-sahman-1",
    "title وب/اپ، OTP SMS template، legal terms — پس از تأیید کارفرما.",
    [
        "بدون تغییر domain تا DNS آماده.",
        "فارسی RTL.",
    ],
    [
        "apps/web/index.html",
        "apps/mobile/driver_app/pubspec.yaml",
        "apps/mobile/community_app/pubspec.yaml",
    ],
    """پروژه: logestic — BRAND-SAHMAN-1 (P1)

## پس از تأیید نام «سهمان»
- web title + favicon
- Flutter app names
- SMS OTP template prefix
- docs/gozaresh-tahvil footer

## DoD
- grep «سهمان» در UI اصلی
- کارفرما visual sign-off""",
    ["سهمان in web/mobile titles"],
    ["visual review"],
)
...
add(
    "DISPATCH-AUTO-PILOT-1",
    "P1",
    "تصمیم dispatch manual vs auto پایلوت",
    "dispatch-auto-pilot-1",
    "DISPATCH_MODE=manual پیش‌فرض — برای پایلوت باید کارفرما تأیید کند auto لازم است یا دستی کافی.",
    [
        "env.ts DISPATCH_MODE",
        "بدون rewrite dispatch engine — فقط config + doc.",
    ],
    [
        "apps/backend/src/config/env.ts",
        "apps/backend/src/services/dispatchService.ts",
    ],
    """پروژه: logestic — DISPATCH-AUTO-PILOT-1 (P1)

## سؤال کارفرما
روز اول UAT: اپراتور دستی dispatch کند یا auto؟

## اگر auto
- env DISPATCH_MODE=auto در staging/production pilot
- test-disp1 regression

## اگر manual (پیش‌فرض)
- document in uat-handover-checklist
- no code change""",
    ["client decision recorded", "env set if auto"],
    ["npm -w @app/backend run test:disp1 if auto"],
)
