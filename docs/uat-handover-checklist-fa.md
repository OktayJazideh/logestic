...
**راهنمای کامل:** [demo-users-and-deploy-fa.md](demo-users-and-deploy-fa.md) (Linux deploy + APK + هر نقش)
...
| OPERATION_ADMIN | 09000000002 | dispatch، monthly-close، approve باسکول |
| COOP_ADMIN | 09000000001 | KYC، تأیید صورت وضعیت |
| COOP_OPERATOR | 09000000111 | **وزن باسکول دستی** (WeighbridgePage) |
| OPERATION_LOCKER | 09000000103 | lock settlement |
| DRIVER | 09000000003 | اپ راننده / FSM |
| FLEET_OWNER | 09000000004 | کیف پول مالک |
| HOUSEHOLD (approved) | 09000001001 | community app — wallet |
| HOUSEHOLD (pending) | 09000000005 | ثبت خانوار |
| CONSULTANT | 09000000006 | تأیید ساعتی (فاز ۲+) |
| OPERATOR | 09000000008 | START/END ساعتی (فاز ۲+) |

- [ ] seed TAFTAN + `service_contract` با `fixed_community_amount_rial_per_unit` (placeholder 400٬000 ریال/تن)
- [ ] seed TAFTAN: `platform_fee_value = 0.01` (split عملیاتی **99/1** — FIN-SEED-99-1 ✓)
...
شامل: infra-regression + comm-ton1 + set1 + **hold-settlement1**

- [ ] `test:infra-regression:finance` — PASS (یا هر script جداگانه §۳.۲–۳.۳)
...
- [ ] `test:pilot-taftan1` — ۳ run PASS (۱۰ گام API؛ assert **99/1**؛ بدون `__dev/seed/demo`)

### ۳.۳b HOLD settlement

```powershell
npm -w @app/backend run test:hold-settlement1
```

- [ ] `test:hold-settlement1` — ۳ run PASS (HELD + PENDING_HOLD excluded)
...
- [ ] split عملیاتی: ~**99٪** مالک + ~**1٪** platform
